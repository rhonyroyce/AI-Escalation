...
The file is organized into the following major sections:
 ...
 14. RENDER FUNCTIONS - PAGE RENDERERS
 ...
 16. CONSOLIDATED PAGE RENDERERS
...
# REMOVED: render_executive_summary — dead code, previously called only from standalone mode
...
# REMOVED: render_benchmarking — dead code, previously called only from standalone mode


# REMOVED: render_root_cause — dead code, previously called only from standalone mode


# REMOVED: render_action_tracker — dead code, previously called only from standalone mode
...
# REMOVED: render_whatif_simulator — dead code, previously called only from standalone mode

# REMOVED: render_drift_page — dead code, previously called only from standalone mode

# REMOVED: render_alerts_page — dead code, previously called only from standalone mode
...
def render_deep_analysis(df):
    """Deep Analysis — orchestrates chart modules across 6 tabs.

    Delegates rendering to focused sub-modules in
    ``escalation_ai.dashboard.charts``.  Each module handles its own
    Streamlit layout and chart creation.
    """
    from escalation_ai.dashboard.charts import (
        render_categories_tab, render_engineers_tab, render_root_cause_tab,
        render_patterns_tab, render_similarity_tab, render_lessons_tab,
    )
    ...
    with tabs[1]:
        render_engineers_tab(df)

    with tabs[2]:
        render_root_cause_tab(df)

    with tabs[3]:
        render_patterns_tab(df)

    with tabs[4]:
        render_similarity_tab(df)

    with tabs[5]:
        render_lessons_tab(df)
...
# REMOVED: render_advanced_insights — dead code, previously called only from standalone mode


# REMOVED: render_dashboard — dead code, previously called only from standalone mode
