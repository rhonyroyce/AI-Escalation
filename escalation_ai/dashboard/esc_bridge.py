...
def esc_load_and_filter(page_name: str = "Executive Dashboard") -> pd.DataFrame:
    ...
    REQUIRED_COLS = {
        'AI_Category': 'AI classification category (run pipeline to generate)',
        'tickets_data_severity': 'Ticket severity level',
        'tickets_data_issue_datetime': 'Issue creation timestamp',
        'Strategic_Friction_Score': 'Composite risk score (run pipeline to generate)',
    }
    ...
    if missing_required:
        st.error(
            ...
            f"These columns are generated by the ML pipeline. "
            f"Run `python run.py --no-gui` to process your data first."
        )
    ...
    if missing_optional:
        st.info(
            f"Some optional columns not found: {', '.join(missing_optional)}. "
            f"Related charts will be disabled. Run the full pipeline for all features."
        )
